...
fashion_train_features = torch.cat([fashion_train[i][0] for i in range(NUM_SAMPLES)], dim=0)
fashion_train_labels = torch.tensor([fashion_train[i][1] for i in range(NUM_SAMPLES)])
fashion_test_features = torch.cat([fashion_test[i][0] for i in range(NUM_SAMPLES)], dim=0)
