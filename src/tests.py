...
def permute_neurons(net):
    ...
    perm = torch.tensor(
        [i for i in range(net.layer_sizes[0])]
    )  # identity permutation for first iteration
    for layer in net.layers:
        out_features = layer[0].out_features
        perm_layer = layer[0].weight[
            :, perm
        ]  # permute layer along second axis according to previous permutation
        perm = torch.randperm(out_features)
        perm_layer = perm_layer[
            perm
        ]  # permute layer along first axis according to current permutation
    ...
    def test_fusion(self):
        ...
        for net_size in [
            ...
            for bias in [True, False]:
                ...
                scaled_net = SimpleNN(
                    net_size, bias=bias, weights=2 * net.get_weight_tensor()
                )
                ...
                self.assertTrue(
                    models_equal(fuse_models(net, perm_net, delta=1), perm_net)
                )
                ...
                self.assertTrue(
                    weights_equal(
                        1.5 * net.get_weight_tensor(),
                        fuse_models(net, scaled_net, delta=0.5).get_weight_tensor(),
                    )
                )
